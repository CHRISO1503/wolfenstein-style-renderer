...
pub fn render(canvas: &mut WindowCanvas, player: &Player) {
    ...
    for i in 0..WINDOW_WIDTH {
        // cast ray
        let ray_direction =
            (player.rotation.y - FOV / 2.0 + FOV * i as f32 / WINDOW_WIDTH as f32) % (2.0 * PI);
        ...
        let ydir;
        if ray_direction < 3.0 * PI / 2.0 && ray_direction > PI / 2.0 {
            ydir = -1;
        } else {
            ydir = 1;
        }
        ...
        let m = match (xdir, ydir) {
            (1, 1) => (PI / 2.0 - ray_direction).tan(),
            (1, -1) => -(ray_direction - PI / 2.0).tan(),
            (-1, -1) => -(PI / 2.0 - ray_direction).tan(),
            (-1, 1) => (ray_direction - PI / 2.0).tan(),
            _ => 0.0,
        };
        ...
        //dist ray (x,y) init at first grid line then dist(x,y) between each grid line. then
        //iterate based in min(x,y), incrementing by dist(x,0) or dist(0,y)
